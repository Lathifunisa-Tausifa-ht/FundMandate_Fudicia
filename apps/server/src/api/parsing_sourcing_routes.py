...
async def ws_parse_mandate_realtime(websocket: WebSocket, session_id: str):
    ...
    try:
        ...
        try:
            ...
            print(f"\n[DEBUG] [WS] About to invoke agent...")
            print(f"[DEBUG] [WS] Input data keys: {input_data.keys()}")
            print(f"[DEBUG] [WS] Config: {config}")
            
            loop = asyncio.get_event_loop()
            try:
                print(f"[DEBUG] [WS] Calling agent.invoke in executor...")
                result = await loop.run_in_executor(
                    None,
                    lambda: agent.invoke(input_data, config)
                )
                print(f"[DEBUG] [WS] agent.invoke completed successfully")
                print(f"[DEBUG] [WS] Result type: {type(result)}")
                print(f"[DEBUG] [WS] Result keys: {result.keys() if isinstance(result, dict) else 'Not a dict'}")
                
            except Exception as e:
                print(f"\n[DEBUG] [WS] ❌ ERROR in agent.invoke: {e}")
                print(f"[DEBUG] [WS] Error type: {type(e).__name__}")
                print(f"[DEBUG] [WS] Error details:")
                import traceback
                traceback.print_exc()
